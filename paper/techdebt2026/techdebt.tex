...
In the second phase, we proposed to students to mitigate the code duplications found by the tool in the IIO subsystem and the AMD Display driver and pass the process of sending the mitigations as a PATCH (\textit{participant-as-observer}).
...
In the \textbf{complete participant observation} phase, the first duplicated function pair chosen was function \textit{offset\_to\_id} on files \textit{dc/gpio/dcn32/hw\_translate\_dcn32.c} and \textit{dc/gpio/dcn315/hw\_translate\_dcn315.c}. The second duplicated function pair was function \textit{phy\_id\_to\_atom} in file 
...
\todo[inline]{Luan, temos alguma evidência nas suas trocas com os mantenedores sobre isso? ou foi algo que veio mais das conversas com o Siqueira?}
