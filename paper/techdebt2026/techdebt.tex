...
%TODO: explicar o processo do envio para o mentores/monitores, o tipo de feedback que recebiam antes enviarem.
...
%TODO: aqui, no caso do Sebastian, também houve a revisão intermediária dos mentores?
...
%TODO: é fácil e queremos disponibilizar esses blogs também no pacote de replicação?

\section{Results}

In the \textbf{complete participant observation} study (first phase), we chose one function pair found on AMD Display Driver to approach and apply the systematic approach.
%
...
Since we suffered a delay in the review process, and to understand why, we directly contacted a maintainer. It was mentioned to us that one reason for the delay was that duplicated code enhances the independence of GPU driver code. This allows developers to make changes to a specific GPU without needing to test compatibility with others, which helps save a significant amount of time and effort. This example of hardware variations, also mentioned by Kapser and Godfrey~\cite{cloneharm}, corroborates our understanding that GPU driver developers do not view duplicated code negatively by default. Thus, as the primary result of our complete participant observation we understood that:

\begin{highlight-box}{Takeaway T1}
  Driver developers do not necessarily view
...
Regarding our analysis from the \textbf{participant-as-observer} phase, we observed that most groups applied the parameterized refactor method while a few used the extractor method.
