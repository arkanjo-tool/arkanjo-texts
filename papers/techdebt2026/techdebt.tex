...
%
Through interactions with maintainers, we gained insights into how the Linux community perceives code quality related to code clones in the project's ``development trenches''. Contrary to the DRY principle, our findings confirm that, in some contexts, maintainers can tolerate duplication to improve readability.  \textbf{We also identified reasons coupled to enhancing performance and avoiding integration overhead}. This empirical work also offers a distinct approach to researching technical debt management practices related to code duplication in FLOSS projects, beyond indicating that deduplication contributions are a viable entry path for newcomers in the Linux kernel.

\section{Related Work}
The works by \citet{ccfinder2002} and \citet{CP-Miner-2006} proposed techniques to identify duplications, using the Linux codebase as a case study. \citet{kapserCloningConsideredHarmful2008} discussed patterns of cloning used in software, including situations where this could be positive, and \citet{rahman2012clonesSmell} contested the notion of clones as a bad smell. \citet{tornhillSoftwareDesignXRays2018} claims that clones evolving independently might not be problematic, whilst \citet{juergensCodeClonesMatter2009} argue that inconsistent changes to clones that should evolve together are what primarily lead to faults. \citet{scsi-cloning-2011} explore the use of cloning as a development technique for the SCSI drivers in the Linux Kernel, where similar devices had drivers born from code cloning. \citet{harderSoftwareClonesGuilty2017} acknowledges the harmful case of cloned bugs but provides empirical evidence contradicting popular beliefs regarding the adverse effects of clones. It did not provide clear guidance for practitioners, leaving the way open for works such as this one.
...
In the \textbf{complete participant observation} study (first phase), we sent the first revision of the patch on August 9, 2024, and a resend followed on October 9, 2024, due to the lack of an initial feedback. Nevertheless, for the first two versions, the maintainers' response requested minor changes regarding coding style, license use, and alignment with existing conventions. In the third version, the feedback was to move the new generic library functions into an existing file rather than creating a new one. Then, the fourth version was accepted and integrated into the kernel on February 25, 2025, removing 406 lines in total and impacting three generations of drivers.~\looseness=-1

Since we experienced a considerable delay in the review process, we directly contacted a maintainer to understand the cause. From this conversation, we learned that it is commonplace for drivers to be cloned (sometimes entirely) to provide a solid starting base for the new driver and to enhance the independence between them, allowing developers to make changes to one without needing to check for regressions on others. This practice is very similar to what was detected in the SCSI subsystem by \citet{scsi-cloning-2011}, agrees with \citet{tornhillSoftwareDesignXRays2018} regarding clones evolving independently, and is also the hardware variation of the forking pattern defined by \citet{kapserCloningConsideredHarmful2008}. Even so, this driver duplication is a \textit{Self-Admitted Technical Debt} (SATD)~\cite{selfAdmittedTd2014} listed as a target for welcomed contributions in the official documentation of the Display Core component\footnote{\url{https://www.kernel.org/doc/html/v6.17/gpu/amdgpu/display/display-contributing.html\#reduce-code-duplication}.}. In this framing, we can attribute the delay to the change impacting multiple drivers, which resulted in greater debt principal on the maintainer’s side to ensure the repayment was stable. Prioritization of other repayments may also have contributed to the delay. With that in mind, this complete participant observation produced our initial takeaway:

\begin{framed}
\noindent\textbf{Takeaway T1 (Driver Forking):} \itshape Driver developers do not necessarily view duplicated code negatively, often consciously cloning entire drivers for many reasons; however, sometimes such duplications are explicitly acknowledged as technical debt, and their repayments are accepted when perceived as safe and stable.
\end{framed}
...
\multirow{ 2}{*}{1} & AMD & 90\% & Accepted (v2) & T1 & +90/-489 \\
...
Table~\ref{tab:stu} summarizes all the contributions sent by the main author and student groups (\textbf{GID})\footnote{We include the first phase experience as Group 0, because the author was a newcomer.}. It contains the information about the chosen subsystem (\textbf{SS}) and the similarity threshold (\textbf{SML}). Each entry also has the \textbf{patch status}, \textbf{takeaways} that may have surfaced from the interaction, and a code differential (\textbf{Diff}) corresponding to the final patch versions before it was dropped or accepted into the code base. The code differential for a set of patches corresponds to the sum of added and removed lines of each patch.
...
In summary, as previously mentioned, T1 and T2 have been reported in the literature, but \textbf{our close ethnographic observation also yields differing and sometimes contradictory perspectives that warrant further investigation}. Additionally, takeaways T3 and T4 go beyond and \textbf{offer nuanced insights that seem specific to debt management decisions that Linux maintainers practice}.

Moreover, another engaging aspect stemming from this work is the realization that deduplication offers interesting opportunities in the Linux kernel and can be an excellent entry path for newcomers. 

Finally, in terms of numbers, the 13 contributions included: (i) a total of 585 lines added and 1,626 lines removed; (ii) 8 contributions were accepted, yielding a success rate of 62\%; (iii) for accepted contributions, the total code diff was +255/-1,152, with an average of +31.9/-144.0 lines added/removed; and (iv) for rejected contributions, the total code diff was +181/-231, averaging +36.2/-46.2 lines added/removed, hinting that smaller duplications are more easily tolerated.
...
Our first two empirical findings have been discussed in the literature on the benefits of duplication: detecting the driver-forking pattern (T1) and duplication motivated by readability (T2). However, contrasting observations across these contexts highlight significant nuances that warrant more in-depth investigation. The latter two findings highlight project-specific debt management in the Linux kernel: change propagation considerably increases the debt principal (T3), and performance preservation in resource-constrained settings (T4) further justifies tolerating clones.

Without disregarding its limitations, this work shows that \textbf{the Linux community has particular technical debt management practices}, carefully weighing the pros and cons of accepting versus repaying duplication debts. Maintainability, clarity, and practical trade-offs shape maintainers' judgments, suggesting that consciously asking \textit{``\textbf{When} do you repeat yourself?''} can be better than mindlessly following the \textit{``Don't Repeat Yourself''} principle.
