...
For instance, the Linux kernel is a foundational \textit{Free/Libre and Open Source Software} (FLOSS) project with more than 27 million lines of code, excluding non-programming code (verifiable through our replication package in Section~\ref{sec:replication-package}), and over 2,000 developers involved in version 6.17~\cite{linux6.17devstats}. Although it is known that some Linux device drivers deliberately duplicate code~\cite{kapserCloningConsideredHarmful2008}, it can be hard to determine why a specific clone exists, even when analyzing upstream commits in the mainline and subsystems. One could argue that the debt interest is too low and the debt principal is too high, or that there is a motive for accepting the clone unrelated to technical debt, or that simply no reasonable deduplication proposal has ever been submitted. We are left to speculate (in a well-informed manner) about the debt management practices of developers, and the chain of thought behind each commit we analyze.
...
For this second phase, our primary data source was the mailing list threads, where all development occurred (also available in our replication package in Section~\ref{sec:replication-package}). Using blog posts, the groups documented their experiences submitting a patch to the Linux kernel, and we also leveraged those to complement our observations.
...
We derived three additional takeaways from our \textbf{participant-as-observer observation}, substantiating them with quotes from the maintainers that emerged during the review process\footnote{We \textbf{do not claim} that these quotes fully represent the maintainers' stance on code duplication or any technical debt management matter.}.

Groups 1 and 4 both refactored similar predicate functions that were logical negations of each other. Group 4 first used macros, but after maintainer feedback, both groups converged on keeping one function and having the other return its negation. Although initially correct, both solutions underwent significant changes driven by maintainer concerns about readability. For Group 1, a maintainer said that \textit{``[...] the naming as \texttt{\_reg\_check()} is not helpful as it doesn't indicate anything specific is being checked.''}, while for Group 4, the feedback explicitly highlighted the precedence of readability over deduplications: \textit{``I think the old code is more readable than hiding the values in a macro even if it is duplicating a few lines of code.''} Even though the deduplication of Group 2 was not similar to that of Group 1 and Group 4 in terms of implementation (it used the parametized method), it also brought forth the readability factor: \textit{``In my view this isn't a significant enough reduction to justify the more complex code.''}


Groups 3, 6, and 10 used a parameterized approach with a new struct as a generic function parameter. The same maintainer reviewed all three, but only Group 3 had its patch accepted. We think this was because it applied to a straightforward iterative algorithm and also fixed a bug, helping its approval. Although deduplication does not always reduce lines, the patch from Group 6 added lines, which the maintainer cited as a drawback. Still, the real reason for the rejection was that the deduplication scattered parts of a complex code that resided close together before: \textit{``Wrapping this up doesn't provide any real advantage, requiring as it does the reviewer to look at this function AND where the value is set rather than seeing them in one place.''} Needing to jump to a function definition (to see its behavior) or a function call (to see its concrete parameters) to understand code is a cognitively demanding task for developers~\cite{questions-programmers-ask}. The reason for rejecting the patch from Group 10 was that the deduplication lowered readability, as the maintainer was \textit{``[...] not sure the code reduction is sufficiently to cover the resulting loss of readability.''}.

These experiences revealed a recurring maintainer concern beyond functional correctness or redundancy reduction: a preference for clones that enhanced code clarity. This may seem counterintuitive, as higher duplication is often linked to lower readability due to the cognitive load of navigating clones~\cite{fowler2018refactoring}. However, the prevalence of small, intra-file duplications in most IIO patches may have shifted the trade-off between clone debt and readability degradation. Based on these consistent insights from multiple reviews, we formulated our second takeaway:

\begin{framed}
\noindent\textbf{Takeaway T2 (Readability):} \itshape Maintainers prioritized readability over removing duplications in many different contexts. Accepting debt from duplication may be worth it if it results in easier-to-read, more understandable code.
\end{framed}
...
More interestingly, the study employs a novel ethnographic approach to engage with technical debt literature that challenges the DRY principle, while refining the discussion within the Linux project. The first two findings show that repayments can be slow due to prioritization and high principal from driver forking (T1), aligning with the existing literature, and that duplication can enhance readability (T2), contradicting other classical studies. The latter two findings highlight project-specific debt management in Linux: change propagation considerably increases principal (T3), and performance preservation in resource-constrained settings (T4) further justifies tolerating clones.
